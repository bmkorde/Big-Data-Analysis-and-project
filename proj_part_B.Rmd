...
air_quality_clean <- air_quality %>%
  ...
  filter(!is.na(pm2_5))  # optional: filter rows with valid PM2.5
...
```{r}
summary(air_quality_clean)
# Replace missing values with column means (or remove)
...
```{r}
# Step 1: Prepare data for time series regression
...
# Step 2: Create lag variables (1-month and 2-month lag)
...
# Step 3: Fit the time series regression model
ts_model <- dynlm(avg_price ~ L(pm2_5_lag1) + L(pm2_5_lag2), data = regression_data)

# Step 4: Review the regression result
summary(ts_model)
...
```{r}
# 1. Create monthly aggregated temperature
monthly_temp <- land_temp_clean %>%
  group_by(date) %>%
  summarise(global_avg_temp = mean(Average_temp, na.rm = TRUE))
rice_with_state_cluster <- rice_with_state_cluster %>%
  mutate(date = as.Date(format(date, "%Y-%m-01")))
# 2. Join global temperature to rice + pollution data
regression_data <- rice_with_state_cluster %>%
  group_by(date) %>%
  summarise(
    avg_price = mean(avg_price, na.rm = TRUE),
...
# --------------------------------------------
# Optional: Combine results in a table
# --------------------------------------------
